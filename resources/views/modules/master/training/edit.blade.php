...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$.ajax({
			url: '{{ url($pageUrl.'check-all/'.$record->id) }}',
			type: "GET",
	...
	$(document).ready(function () {
		var showuserurl = '{{ url($pageUrl.'show-users/'.$record->id.'/'.$record->site_id) }}';
			...
			<div class="ui form">
				...
				<div class="ui attached active tab segment" data-tab="first">
					...
					<div class="field">
						...
						<textarea name="contents" placeholder="Contents">{!! $record->contents or '' !!}</textarea>
					</div>
					<div class="field">
						<label>Company</label>
					...
					<div class="field">
						...
						<select name="type_training_id" class="ui search selection dropdown">
							{!! \App\Models\Master\TypeTraining::options('name', 'id', ['selected' => $record->type_training_id], 'Choose Type Training') !!}
						</select>
					</div>
				</div>
